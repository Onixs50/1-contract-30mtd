...
def print_boxed_message(message, color='cyan', width=60):
    print(colored("+" + "-" * width + "+", color))
    message_lines = [message[i:i+width-4] for i in range(0, len(message), width-4)]
    for line in message_lines:
        print(colored(f"| {line:<{width-4}} |", color))
    print(colored("+" + "-" * width + "+", color))
...
def manage_network_info():
    ...
    print_boxed_message("Available Networks", 'yellow')
    for i, (chain_id, network) in enumerate(networks.items(), 1):
        print(colored(f"{i}. Chain ID: {chain_id}", 'cyan'))
        print(colored(f"   RPC: {network['rpc']}", 'cyan'))
        print(colored(f"   Explorer: {network['explorer']}", 'cyan'))
        print()
    ...
    while True:
        ...
        if choice.lower() == 'a':
            ...
            print_boxed_message(f"Network with Chain ID {chain_id} added successfully.", 'green')
            ...
            print_boxed_message("Invalid choice. Please try again.", 'red')
...
def display_wallet_balances(web3, private_keys):
    print_boxed_message("Wallet Balances", 'blue')
    for i, private_key in enumerate(private_keys, 1):
        account = web3.eth.account.from_key(private_key)
        balance = web3.eth.get_balance(account.address)
        balance_eth = web3.from_wei(balance, 'ether')
        print(colored(f"Wallet {i}: {account.address}", 'cyan'))
        print(colored(f"Balance: {balance_eth:.6f} ETH", 'yellow'))
...
def get_gas_settings(web3):
    ...
    print_boxed_message("Current Gas Settings", 'yellow')
...
def deploy_contract(web3, private_key, explorer_url):
    ...
    print_boxed_message(f"Contract deployed at address: {tx_receipt.contractAddress}", 'green')
...
def interact_with_contract(web3, contract_address, abi, private_key, interaction_count, min_delay, max_delay, explorer_url):
    ...
    for i in range(interaction_count):
        ...
        print_boxed_message(f"Interaction {i+1}/{interaction_count}: {method.fn_name}", 'yellow')
        ...
        try:
            ...
            failed += 1
            print_boxed_message(f"Transaction failed: {str(e)}", 'red')
...
def main():
    clear_screen()
    print_boxed_message("Advanced Wallet Interaction Script by onixia", 'yellow', 70)
    ...
    if not web3.is_connected():
        print_boxed_message("Failed to connect to the network. Please check your RPC URL.", 'red')
        return

    private_keys = manage_private_keys()

    # Display wallet balances before starting interactions
    display_wallet_balances(web3, private_keys)
    ...
    input(colored("Press Enter to start interactions...", 'green'))
    ...
    print_boxed_message("Script completed by onixia", 'yellow', 70)
